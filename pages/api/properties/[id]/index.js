...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (!property) {
        return res.status(404).json(backupProperty);
  ...
  // }
    ...
    if (req.body.data) {
      const { imageUrl, id } = req.body.data;
      ...
      try {
        ...
        const updatedData = {
          ...
          bathrooms:
            bathrooms !== '' ? parseFloat(bathrooms) : property.bathrooms,
          ...
          sewer: sewerBoolean,
          antiquity:
            antiquity !== '' ? parseFloat(antiquity) : property.antiquity,
        ...
        return res
          .status(200)
          .json({ message: 'Property actualizado con éxito' });
      } catch (error) {
